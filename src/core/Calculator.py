...
from src.buttons.ActionButton import ActionButton
from src.buttons.DigitButton import DigitButton
from src.buttons.ExtraActionButton import ExtraActionButton
from src.buttons.HistoryButton import HistoryButton
from src.exceptions.InvalidExpressionException import InvalidExpressionException
...
class Calculator(ft.Container):
    ...
    def __init__(self):
        ...
        self.content = ft.Column(controls=[
            ft.Row(controls=[HistoryButton()]),
            ft.Row(controls=[self.expression], alignment="end"),
            ft.Row(controls=[self.result], alignment="end"),
            ft.Row(controls=[
                ActionButton(text="v-", button_clicked=self.button_clicked),
                ActionButton(text="sen", button_clicked=self.button_clicked),
    ...
    def button_clicked(self, e):
        ...
        if self.result.value == "Error" or data == "AC":
            ...
            HistoryStorage.add_history(self.expression.value,
                                       self.result.value)

        elif data in ("%"):
            self.result.value = self.result.value.replace(" ", "")
            self.result.value = str(float(self.result.value) / 100)
            self.result.value = "{:,.2f}".format(float(
    ...
    def calculate(self, expression):
        try:
            ...
            e = InvalidExpressionException(f"Invalid expression: {expression}",
                                           self.logger)
            e.error()
            result = "Syntax Error"
